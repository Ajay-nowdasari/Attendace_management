...
`;
  ...
  useEffect(() => {
    const fectchUser = async () => {
      ...
      if (usertype !== "user") {
        navigate("/");
        toast.warn(" You must be logged in as Student to access this page.")
      }
      try {
        const response = await axios.get(`${API_BASE_URL}single_user_profile/`, {
          headers: {
  ...
  const handleEdit = () => {
    ...
    setIsEdit(!isedit);
    if (isedit) {
      setShowEditInfoModal(true); // Show the modal
    }
  }
  useEffect(() => {
    console.log("Student ID:", userId); // Add this line to debug
    if (userId) {
      axios.get(`${API_BASE_URL}student/${userId}/attendance/`)
        .then(response => {
  ...
  return (
    <div>
      <ToastContainer />
      <Modal show={true} style={{ top: "0" }} className={`std_Attendance_modal ${darkMode ? 'dark-mode' : 'light-mode'}`} centered>
            ...
            <tbody className='table-secondary'>
              {attendance.length === 0 ? (
                ...
                attendance.map((record, index) => {
                  const presentDays = record.present_days;
                  const workingDays = presentDays + record.absent_days;
                  const attendancePercentage = ((presentDays / workingDays) * 100).toFixed(2);
                  const monthName = getMonthName(record.month); // Convert month number to name
        ...
        <Modal.Header className="px-4" style={{ backgroundColor: "#3f4961", color: "#c3c7d3" }}>
          <Modal.Title>
            ...
            <div className="profile_mdl_hdr" style={{ alignItems: "center" }}>
              ...
              <svg className='pt-4' width="150px" height="200px" viewBox="0 0 61.7998 61.7998" xmlns="http://www.w3.org/2000/svg">
              ...
              <h2 className='px-4 pt-4' align="center">{user.name}</h2>
            </div>
          </Modal.Title>
        </Modal.Header>
